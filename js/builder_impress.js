...
Impress = (function (impdoc, impwin) {
    ...
    var startup = (function () { // impdoc, impwin) {
        ...
        var impress = impwin.impress = function (rootId) {
            ...
            var canvas = impdoc.createElement("div");
            ...
            var init = function () {
                ...
                var meta = $("meta[name='viewport']") || impdoc.createElement("meta");
                meta.content = "width=device-width, minimum-scale=1, maximum-scale=1, user-scalable=no";
                if (meta.parentNode !== impdoc.head) {
                    meta.name = 'viewport';
                    impdoc.head.appendChild(meta);
                }
                ...
                // set initial styles
                impdoc.documentElement.style.height = "100%";
            ...
            var goto = function (el, duration) {
                ...
                impwin.clearTimeout(stepEnterTimeout);
                stepEnterTimeout = impwin.setTimeout(function () {
            ...
            root.addEventListener("impress:init", function () {
                ...
                root.addEventListener("impress:stepenter", function (event) {
                    impwin.location.hash = lastHash = "#/" + event.target.id;
                }, false);

                impwin.addEventListener("hashchange", function () {
                    // When the step is entered hash in the location is updated
                    // (just few lines above from here), so the hash change is 
                    // triggered and we would call `goto` again on the same element.
                    //
                    // To avoid this we store last entered hash and compare.
                    if (impwin.location.hash !== lastHash) {
    ...
    var startupNav = (function () { //(impdoc, impwin) {
        ...
        impdoc.addEventListener("impress:init", function (event) {
            ...
            impdoc.addEventListener("keydown", function (event) {
            ...
            impdoc.addEventListener("keyup", function (event) {
                if (event.keyCode === 9 || (event.keyCode >= 32 && event.keyCode <= 34) || (event.keyCode >= 37 && event.keyCode <= 40)) {
            ...
            impdoc.addEventListener("click", function (event) {
                // event delegation with "bubbling"
                // check if event target (or any of its parents is a link)
                var target = event.target;
                while ((target.tagName !== "A") &&
                    (target !== impdoc.documentElement)) {
                ...
                if (target.tagName === "A") {
                    ...
                    if (href && href[0] === '#') {
                        target = impdoc.getElementById(href.slice(1));
                    }
                }
            ...
            impdoc.addEventListener("touchstart", function (event) {
                if (event.touches.length === 1) {
                    var x = event.touches[0].clientX,
                        width = impwin.innerWidth * 0.3,
                        result = null;

                    if (x < width) {
                        result = api.prev();
                    } else if (x > impwin.innerWidth - width) {
                        result = api.next();
                    }
            ...
            impwin.addEventListener("resize", throttle(function () {
                // force going to active step again, to trigger rescaling
                api.goto(impdoc.querySelector(".active"), 500);
            }, 250), false);
